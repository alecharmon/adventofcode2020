...
func (s *ship) exec(i instruction) {
	...
	default:
		s.waypoint.moveInDir(dirs[i.action], i.value)
	}
	fmt.Println(i, s.currentDir, s.position)
}

func (w *waypoint) moveInDir(dir direction, value int) {
	if w.position[dir.opposite] > 0 {
		diff := w.position[dir.opposite] - value
		if diff < 0 {
			w.position[dir.opposite] = 0
			w.position[dir.value] = adventofcode2020.Abs(diff)
		} else {
			w.position[dir.opposite] = diff
		}
	} else {
		w.position[dir.value] = w.position[dir.value] + value
	}
}
...
func createShip(dir string) *ship {
	return &ship{
		...
		waypoint:   &waypoint{map[string]int{"E": 10, "N": 1}},
	}
}
